...
export default function App() {
  ...
  return (
    <>
      {!isLoggedIn ? (
        authView === 'login' ? (
          <LoginPage onLogin={() => setIsLoggedIn(true)} onSignup={() => setAuthView('signup')} />
        ) : (
          <SignupPage onSignup={() => setAuthView('login')} onBack={() => setAuthView('login')} />
        )
      ) : (
