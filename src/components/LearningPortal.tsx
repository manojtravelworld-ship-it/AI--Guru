...
export const LearningPortal: React.FC<LearningPortalProps> = ({
  ...
  const handlePdfUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Keep existing logic for text cleaning
      const cleaned = text.replace(/[^\x20-\x7E\n\r\t]/g, ' ').replace(/\s{3,}/g, '\n').slice(0, 8000);
      setPdfExtractedText(cleaned.length > 100 ? cleaned : null);
...
<body>
  ...
  <p><strong>AI-GURU Interactive Simulation Package</strong></p>
  <div class="card">
    <h3>Objectives</h3>
    <ul>
  ...
  const handleExportPptx = useCallback(async () => {
    ...
    try {
      ...
      titleSlide.addText('Generated by AI-GURU', {
  ...
  const allReviewed = Object.values(reviewStatus).every(Boolean);
  const toggleReview = (key: keyof typeof reviewStatus) => {
    setReviewStatus(prev => ({ ...prev, [key]: !prev[key] }));
  };
  ...
  const handleSaveToDrive = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/save-to-drive', {
        ...
        body: JSON.stringify({ 
          token: accessToken, 
          fileName: `${generatedLessonSuite.title.replace(/\s+/g, '_')}_lesson.json`, 
          content: JSON.stringify(generatedLessonSuite, null, 2) 
        }),
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-slate-900/80 border border-slate-800 rounded-2xl p-4 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-black tracking-wider text-slate-100 uppercase">AI-GURU Immersive Portal</h1>
            <p className="text-[10px] text-slate-400 font-medium">Multi-Agent Interactive Classroom (OpenVidya Edition)</p>
          </div>
        </div>

        {/* Filter UI */}
        <div className="flex gap-4 p-4 border border-slate-800 rounded-xl bg-slate-900/50 w-full">
          <input 
            type="text" 
            value={grade} 
            onChange={(e) => setGrade(e.target.value)} 
            placeholder="Grade (e.g. 10)" 
            className="bg-slate-800 text-white p-2 rounded border border-slate-700 w-full"
          />
          <input 
            type="text" 
            value={syllabus} 
            onChange={(e) => setSyllabus(e.target.value)} 
            placeholder="Syllabus (e.g. CBSE)" 
            className="bg-slate-800 text-white p-2 rounded border border-slate-700 w-full"
          />
        </div>
      ...
      <div className="p-4 border border-slate-800 rounded-xl bg-slate-900/50">
        <h2 className="text-sm font-bold text-white mb-2">Available Study Materials</h2>
        <MaterialList grade={grade} syllabus={syllabus} />
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col gap-6">
            <div>
              ...
              <div className="space-y-4 mt-4">
                ...
                <div className="border-2 border-dashed border-slate-800 hover:border-indigo-500/50 bg-slate-950/40 rounded-xl p-4 transition-all text-center">
                  ...
                  <p className="text-[9px] text-slate-500 mt-1">AI-GURU extracts concepts automatically to construct modules</p>
            ...
            <div className="border border-slate-800/80 bg-slate-950/40 rounded-xl p-4 space-y-3">
              <h4 className="text-[10px] font-black uppercase text-amber-500 tracking-wider">Review Required</h4>
              <p className="text-[9px] text-slate-500 leading-normal mb-2">Review all materials before publishing.</p>
              <div className="space-y-1.5">
                {Object.entries(reviewStatus).map(([key, reviewed]) => (
                  <label key={key} className="flex items-center gap-2 text-[10px] text-slate-300 cursor-pointer hover:text-white">
                    <input type="checkbox" checked={reviewed} onChange={() => toggleReview(key as any)} className="accent-amber-500 w-3 h-3" />
            ...
            {/* EXPORT OPTIONS BOX */}
            <div className={`border border-slate-800/80 bg-slate-950/40 rounded-xl p-4 space-y-3 ${!allReviewed ? 'opacity-50 pointer-events-none' : ''}`}>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button
                  ...
                  disabled={isSavingToDrive || !generatedLessonSuite}
                  className="px-3 py-2 bg-slate-900 hover:bg-slate-850 text-slate-300 hover:text-white border border-slate-800 rounded-xl transition text-[9px] font-bold uppercase flex items-center justify-center gap-1.5 disabled:opacity-40"
                >
