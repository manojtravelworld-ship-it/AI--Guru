...
interface KnowledgePortalProps {
  currentView: string; // 'ncert-library' | 'scert-library' | 'knowledge-vault' | 'revision-room' | 'lesson-plan' | 'lab-manual'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex border-b border-slate-800 pb-2 overflow-x-auto gap-4">
        {['ncert-library', 'scert-library', 'knowledge-vault', 'revision-room', 'lesson-plan', 'lab-manual'].map((tab) => (
          <button
            key={tab}
            onClick={() => {
              onViewChange(tab);
      ...
      {/* KNOWLEDGE VAULT (Includes Student Notes) */}
      ...
      {/* OTHER PLACEHOLDERS */}
      {(currentView === 'revision-room' || currentView === 'lesson-plan' || currentView === 'lab-manual') && (
        <div className="text-center py-20 text-slate-500">
          <FileText className="w-12 h-12 mx-auto mb-3" />
          <p className="text-sm">The {currentView.replace('-', ' ')} view is currently under development.</p>
        </div>
      )}
    </div>
  );
